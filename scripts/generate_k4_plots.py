...
# ══════════════════════════════════════════════════════════════════════════════
# PLOT k4-07: 5-Fold CV metrics summary (k=4 only, all 5 metrics per fold)
# ══════════════════════════════════════════════════════════════════════════════
def plot_cv_metrics_k4():
    ...
    for i, (m, col) in enumerate(zip(metrics, MCOLS)):
        offset = (i - 2) * w
        bars   = ax.bar(x + offset, fdf[m], w, label=m,
                        color=col, edgecolor="black", lw=0.7, alpha=0.88)
    # Mean line
    means = fdf[metrics].mean()
    for i, (m, col) in enumerate(zip(metrics, MCOLS)):
        ax.axhline(means[m], color=col, lw=1.5, ls=":", alpha=0.7)

    ax.set_xticks(x); ax.set_xticklabels(fdf["Fold"], fontsize=11)
    ax.set_ylabel("Score (%)"); ax.set_ylim(85, 105)
    ax.set_title("k=4 QFS Subset: Per-Fold Metrics (5-Fold Stratified CV)\n"
                 "Dotted lines = mean across folds",
                 fontweight="bold")
    ax.legend(loc="lower right"); ax.grid(axis="y", ls="--", alpha=0.4); spine_off(ax)
    plt.tight_layout()
