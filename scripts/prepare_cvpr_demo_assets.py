...
def cotton_candidate_boxes(img_rgb: np.ndarray, label: str) -> list[tuple[int, int, int, int]]:
    ...
    thresh_val = max(5, min(30, int(otsu_val * 0.12))) 
    ...
    for contour in contours:
        ...
        if float(np.mean(region_s)) > 0.90 and label == "Pre_Defoliation": continue
        if label == "Pre_Defoliation" and float(np.mean(region_exg)) > 0.95: continue
