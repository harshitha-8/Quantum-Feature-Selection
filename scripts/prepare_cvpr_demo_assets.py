...
def cotton_candidate_boxes(img_rgb: np.ndarray, label: str) -> list[tuple[int, int, int, int]]:
    ...
    for contour in contours:
        ...
        if float(np.mean(region_s)) > 0.75 and label == "Pre_Defoliation":
            continue
        if float(np.mean(region_gray)) < 0.10:
            continue
        if label == "Pre_Defoliation" and float(np.mean(region_exg)) > 0.85:
            continue
...
def draw_detection_overlay(base_rgb: np.ndarray, boxes: list[tuple[int, int, int, int]]) -> Image.Image:
    ...
    text = f"Total Bolls Detected: {len(boxes)}"
    font_scale = 3.5
    txt_thick = 8
    (tw, th), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_DUPLEX, font_scale, txt_thick)
    x_txt, y_txt = 80, 160
    cv2.rectangle(out, (x_txt - 20, y_txt - th - 20), (x_txt + tw + 20, y_txt + baseline + 20), (0, 0, 0), -1)
    cv2.putText(out, text, (x_txt, y_txt), cv2.FONT_HERSHEY_DUPLEX, font_scale, (255, 255, 255), txt_thick)
